...
export default async function ProfileCard({params}: {params: Promise<{ id: string }>}) {
  ...
  const loggedInUserProfile : LoggedUserProfile = await getMyProfile();
  const userPosts: Post[] = await getUserPosts(id);

  const profileData = loggedInUserProfile?.user._id === id ? loggedInUserProfile : userProfile ;

  return (
      <ProfileUI  userProfile = {profileData} userPosts = {userPosts} loggedInUserId = {loggedInUserProfile?.user._id}/>
  );
}